...
12. The type layer: extended REA makes types first-class — ResourceType, EventType, AgentType, CommitmentType as records carrying class-level attributes, joined by typification (instance→type) and characterization (type→type, where policies live: "what could or should be" governing "what is"; Geerts & McCarthy 2002, 2006). openREA deliberately flattens this to classification strings (`kind`, `event_type`, `commitment_type`) plus `Policy.applies_when` matching on them. Is that flattening a permanent position or a v0.1 economy? The open thesis: type images existed to give pre-AI systems predictability at the knowledge level — versioned policy-as-code evaluated by reasoning agents may provide the same governance without a parallel type ontology to maintain. If that's wrong, where does it break first — class-level attributes (credit limits per customer type), type-to-type control policies, or standardization of planned occurrences?
