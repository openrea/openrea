...
11. Exchange completeness — the REA axioms: Geerts & McCarthy (2000) state three axioms for full-REA models: (1) every economic resource has at least one inflow event and one outflow event; (2) every outflow event is **eventually** paired in duality with an inflow event, and vice versa — "eventually" is load-bearing, since claims and open commitments live in the gap; (3) every exchange has both inside and outside participation. openREA currently enforces validity, not completeness: duality and reciprocal (§4.9) are optional, and nothing checks the axioms. Should openREA adopt them as an **optional completeness conformance tier** — a check over a record set as of a time, with the legitimate exceptions carved out: congruent exchanges (one event carrying both give and take flows, where the duality link is absorbed by design; G&M 2000), matching too tenuous below aggregate level, gains/losses as isolated legs (McCarthy 1982 pp. 573–575), and unilateral or non-reciprocal commitments? And if the axioms become checkable, does axiom 2's "eventually" get an operator — the un-dualized outflow *is* the open claim, which is how CREASY derived receivables from the event graph (Geerts & McCarthy 1999/2000)?
12. The type layer and intensional subjects: extended REA makes types first-class — ResourceType, EventType, AgentType, CommitmentType carrying class-level attributes, joined by typification (instance→type) and characterization (type→type), so policies can say "what could, should, or must be" about classes (Geerts & McCarthy 2002, 2006). openREA deliberately flattens this to classification strings (`kind`, `event_type`, `commitment_type`) plus `Policy.applies_when` matching on them. Mapping Geerts & McCarthy's three policy kinds (2006) against that flattening locates exactly where it bites: *knowledge-intensive descriptions* are covered (`applies_when` → `outputs` is inference-from-classification); *validation rules* are mechanically covered (conformance checks plus the §13 proposal gate); but *target descriptions* — budgets and standards — break, because a target is a claim about a **class** of (often future) occurrences ("Q3 sales of resource kind F by agent S"), and assertion subjects can only reference records. The candidate resolution is **intensional subjects**: allow an assertion's `subject` to be a versioned, deterministic predicate over records, with membership derived at evaluation time — a budget becomes an estimate-kind assertion over a predicate, a standard becomes a target description over one, and variance analysis becomes discrepancy between predicate-scoped actuals and the target assertion. Geerts & McCarthy license the move themselves: grouping membership "determined procedurally" rather than stored (2006, Fig. 17). Still open: is a predicate language expressive enough to replace typification and grouping outright; what constraints keep intensional subjects deterministic, versioned, and replay-safe (they would need Policy-grade versioning discipline, or historical reconstruction breaks); and do class-level attributes (the credit limit per customer type) still need a home of their own, or are they simply knowledge-intensive policies under another name?
